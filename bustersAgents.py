...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for p in livingGhostPositionDistributions:
            distr = p
            for choice in distr:
                if distr[choice] > maxProb:
                    maxProb = distr[choice]
                    bestChoice = choice

        minDist = {}
        minAction = None
        for l in legal:
            successor = Actions.getSuccessor(pacmanPosition, l)
            if self.distancer.getDistance(successor, bestChoice) < minDist:
                minDist = self.distancer.getDistance(successor, bestChoice)
                minAction = l
        return minAction
