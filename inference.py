...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for p in self.legalPositions:
                ...
                P_of_noisy_given_true_dist = emissionModel[trueDistance]
                newDistribution[p] = P_of_noisy_given_true_dist*self.beliefs[p]
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        "*** YOUR CODE HERE ***"
        newDistribution = util.Counter()
        cacheTable = util.Counter()
        for oldPos in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            cacheTable[oldPos] = newPosDist
        for p in self.legalPositions: #p is GhostLocation_t
            for oldPos in self.legalPositions:
                newDistribution[p] = newDistribution[p] + cacheTable[oldPos][p]*self.beliefs[oldPos]
        self.beliefs = newDistribution
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        particles_per_loc = self.numParticles / len(self.legalPositions)
        self.particleList = []
        for p in self.legalPositions:
            for i in range(0, particles_per_loc):
                self.particleList.append(p)

        modParticles = self.numParticles % len(self.legalPositions)


        counter = 0
        for p in self.legalPositions:
            if counter >= modParticles:
                break
            self.particleList.append(p)
            counter = counter + 1
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            newPosition = self.getJailPosition()
            newList = []
            newWeights = []
            ...
            weightList = util.Counter()
            for i in range(0, self.numParticles):
                trueDistance = util.manhattanDistance(self.particleList[i], pacmanPosition)
                weightList[i] = emissionModel[trueDistance]

            if weightList.totalCount() == 0:
                self.initializeUniformly(gameState)
                
                return

            items = weightList.items()
            values = [i[0] for i in items]

            
            results = util.nSample(weightList, values, self.numParticles)
            newList = a = [0] * self.numParticles
            count = 0
            for r in results:
                newList[count] = self.particleList[r]
                count = count + 1
            self.particleList = newList
